...
- Command `droidscript-code.connect` launches `connect-to-droidscript.js`, which cycles through IP:port endpoints in `serverIPs[]` from `dsconfig.json` (history capped at 10), updating the status bar for each attempt. Clicking the status bar's `Trying` message cancels the loop and immediately reopens the endpoint picker. The picker lists past endpoints and accepts new entries without a port, defaulting to `:8088` or the current `PORT`; manual selections attempt only the chosen endpoint and reopen the picker on failure【F:src/commands/connect-to-droidscript.js†L60-L69】【F:src/commands/connect-to-droidscript.js†L40-L69】.
