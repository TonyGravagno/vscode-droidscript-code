...
- Command `droidscript-code.connect` launches `connect-to-droidscript.js`, which cycles through IP:port endpoints in `serverIPs[]` from `dsconfig.json`, updating the status bar for each attempt and prompting only after all stored entries fail【F:src/commands/connect-to-droidscript.js†L60-L69】.
