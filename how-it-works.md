...
- Command `droidscript-code.connect` launches `connect-to-droidscript.js`, which cycles through IP:port endpoints in `serverIPs[]` from `dsconfig.json` (history capped at 10), updating the status bar for each attempt and prompting only after all stored entries fail. The prompt is an editable quick pick that lists past endpoints and accepts new entries without a port, defaulting to `:8088` or the current `PORT`【F:src/commands/connect-to-droidscript.js†L60-L69】【F:src/commands/connect-to-droidscript.js†L40-L69】.
