...
const axios = {
  ...
  get: async (url, config) =>
    axios.intercept(url, await _axios.get(url, config)),

  /** @type {typeof _axios.post} */
  post: async (url, data, config) =>
    axios.intercept(url, await _axios.post(url, data, config)),
};
...
async function login(password) {
  ...
  try {
    ...
    return /** @type {{status:"bad", error:any}} */ ({
      status: "bad",
      error: error,
    });
  }
...
async function listFolder(folder) {
  ...
  const url = `${DSCONFIG.serverIP}/ide?cmd=list&dir=${querystring.escape(
    folder
  )}`;
...
async function createApp(name, type, template) {
  const url = `${DSCONFIG.serverIP}/ide?cmd=add&prog=${querystring.escape(
    name
  )}&type=${type}&template=${querystring.escape(template)}`;
  const response = await axios.get(url).catch(catchError);
  return response.data;
}
...
async function renameFile(file, newname) {
  const url = `${DSCONFIG.serverIP}/ide?cmd=rename&file=${querystring.escape(
    file
  )}&newname=${querystring.escape(newname)}`;
  const response = await axios.get(url).catch(catchError);
...
async function deleteFile(file) {
  const url = `${DSCONFIG.serverIP}/ide?cmd=delete&file=${querystring.escape(
    file
  )}`;
...
async function play(appName) {
  ...
  const playUrl = `${DSCONFIG.serverIP}/ide?cmd=run&prog=${querystring.escape(
    appName
  )}`;
  try {
...
async function getProjectInfo(dir, title, existFn) {
  ...
  for (const type of types) {
    if (await existFn(`${dir}/${title}.${type}`)) projType = type;
  }
...
async function uploadFile(filePath, folder, fileName) {
  ...
  formData.append(folder, fs.createReadStream(filePath), {
    filename: fileName,
  });
...
async function execute(mode, code) {
  ...
  const url = `${
    DSCONFIG.serverIP
  }/ide?cmd=execute&mode=${mode}&code=${querystring.escape(encodedData)}`;
...
async function exec(code) {
  const url = `${DSCONFIG.serverIP}/ide?cmd=exec&code=${querystring.escape(
    code
  )}`;
...
async function executeCommand(cmd) {
  const url = `${DSCONFIG.serverIP}/ide?cmd=execute&code=${querystring.escape(
    cmd
  )}`;
  const response = await axios.get(url).catch(catchError);
