...
let cancelTry = false;
/** Flag to just try the top entrypoint */
let tryFirstEntrypoint = false;
...
module.exports = async function (callback, status) {
  ...
  if (!DSCONFIG.serverIP)
    tryFirstEntrypoint = await vscode.commands.executeCommand(
      "droidscript-code.selectDevice"
    );
  await tryEndpoints();
  //       await connectWith(DSCONFIG.serverIP.replace(/https?:\/\//, '').split(':')[0], DSCONFIG.PORT, true);
};
...
async function tryEndpoints() {
  const onlyTryFirst = tryFirstEntrypoint;
  tryFirstEntrypoint = false;
  ...
  for (let i = 0; i < endpoints.length; i++) {
    if (cancelTry) {
      ...
      if (tryFirstEntrypoint) tryEndpoints();
      return;
    }
...
async function login(pass = "") {
  ...
  if (!data) {
    // might change selection rather than using separate flag
    let selection = await vscode.window.showWarningMessage(
      "IP Address cannot be reached.",
      "Retry",
      "Re-enter IP Address"
    );
    // user intentionally wants to try a different address
    if (selection === "Re-enter IP Address")
      selection = (await vscode.commands.executeCommand(
        "droidscript-code.selectDevice"
      ))
  ...
  if (data.status !== "ok")
    return await showPasswordPopup(
      "Password is incorrect.",
      "Re-enter password"
    );

  // to be used in DroidScript CLI
