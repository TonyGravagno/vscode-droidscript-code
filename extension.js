...
async function activate(context) {
  ...
  subscribe(
    "revealExplorer",
    (/** @type {ProjectsTreeData.ProjItem} */ treeItem) => {
  ...
  if (CONSTANTS.VERSION > DSCONFIG.VERSION || !assetsExist || CONSTANTS.DEBUG) {
    ...
    if (!CONSTANTS.DEBUG)
      vscode.commands.executeCommand(
        ...
        vscode.Uri.file(__dirname + "/Highlights.md")
      );
  }
...
async function prepareWorkspace() {
  ...
  if (proj.reload) {
    ...
    const selection = await vscode.window.showInformationMessage(
      proj.PROJECT + " is a DroidScript app.\nConnect to DroidScript?",
      "Proceed"
    );
...
const syncActions = {
  updLocal: {
    icon: "$(chevron-down)",
    text: "Update Local",
    desc: "downloads only remote files that already exists locally",
  },
  dnlAll: {
    icon: "$(fold-down)",
    text: "Download All",
    desc: "downloads all files from the remote",
  },
  updRemote: {
    icon: "$(chevron-up)",
    text: "Update Remote",
    desc: "uploads only files that already exist on the remote",
  },
  uplAll: {
    icon: "$(fold-up)",
    text: "Upload All",
    desc: "uploads all local files to the remote",
  },
...
async function loadFiles(action) {
  ...
  if (PROJECT) {
    if (syncButton)
      syncButton.text = `$(sync~spin) ${syncActions[action].text}`;
    await vscode.window.withProgress(
      ...
      async (proc) =>
        await getAllFiles(PROJECT, proc, action || lastSyncAction).catch(
          catchError
        )
    );
    ...
    vscode.window.showInformationMessage(
      "Open an app in the DroidScript's 'PROJECT' section."
    );
  }
}
...
async function indexFolder(
  ...
  for (var i = 0; i < data.list.length; i++) {
    let file = data.list[i],
      path = folder + file;
    // ignore hidden files
    if (excludeFile(conf, path.replace(root, ""))) continue;

    if (file.indexOf(".") > 0) files.push(path);
    else if (!excludeFile(conf, path.replace(root, "") + "/"))
      files.push(
        ...(await indexFolder(path + "/", conf, proc, listFolder, root))
      );
  }
  return files;
}
...
async function getAllFiles(folder, proc, action) {
  ...
  let remoteFiles = (
    await indexFolder(folder, conf, proc).catch((e) => (catchError(e), []))
  ).map((f) => f.replace(PROJECT + "/", ""));
  let localFiles = (
    await indexFolder(proj.path, conf, proc, async (p) => ({
      status: "ok",
  ...
  if (download) {
    ...
    for (const folder of folders) if (folder != ".") createFolder(folder);
...
async function onDidSaveTextDocument(doc) {
  ...
  await batchPromises(fileList, async (doc) => {
    ...
    await ext.uploadFile(
      doc.uri.fsPath,
      path.dirname(dsFile),
      path.basename(dsFile)
    );
  });
}
...
async function onCreateFile(e) {
  ...
  await batchPromises(fileList, async (file) => {
    ...
    if (stats.isFile()) {
      const response = await ext.uploadFile(
        file.fsPath,
        path.dirname(dsFile),
        path.basename(dsFile)
      );
...
async function onRenameFile(e) {
  if (e)
    filesToRename.push(
      ...e.files.filter(
        (f) =>
          getFileProject(f.oldUri.fsPath) && getFileProject(f.newUri.fsPath)
      )
    );
...
function displayControlButtons() {
  ...
  if (!syncButton) {
    syncButton = vscode.window.createStatusBarItem(
...
/**
 * Temporarily display a connection message.
 * @param {string} [msg]
 */
function setConnectionMessage(msg) {
  if (!connectionStatusBarItem) {
    connectionStatusBarItem = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right
    );
...
function setProjectName(project = "") {
  ...
  if (!projectName)
    projectName = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Left
    );
...
async function play(APPNAME) {
  ...
  if (!APPNAME && !PROJECT)
    return vscode.window.showWarningMessage("No Project Selected");
...
function runSampleProgram(treeItem) {
  ...
  if (title.includes("♦")) {
    return vscode.window.showWarningMessage(
      "PREMIUM FEATURE. Please subscribe to 'DroidScript Premium' to run this sample."
    );
  }
...
async function deleteAppDialog(item) {
  ...
  if (!appName)
    return vscode.window.showWarningMessage(
      "Selec an app in DroidScript's PROJECTS section!"
    );
...
async function renameAppDialog(item) {
  ...
  if (!appName || !proj)
    return vscode.window.showWarningMessage(
      "Rename an app in DroidScript section under Projects view!"
    );

  const res = await renameApp(item);
  if (res?.status === "error")
    vscode.window.showErrorMessage("Failed to rename app: " + res.error);
  if (res?.status !== "ok") return;

  projectsTreeDataProvider.refresh();
  vscode.window.showInformationMessage(
    `${appName} successfully renamed to ${res.name}.`
  );
...
async function execCommand() {
  ...
  const items = [
    ...
    {
      label: "!addpackage [url]",
      description: "Add a package with the specified URL",
    },
    {
      label: "!getperms",
      description: "Get all detected permissions used by your app",
    },
    {
      label: "!addplugin [name]",
      description: "Add a plugin with the specified name",
    },
    {
      label: "!addmodule [name]",
      description: "Add a Node.js module with the specified name",
    },
    {
      label: "!remplugin [name]",
      description: "Remove a plugin with the specified name",
    },
    {
      label: "!buildapk [pkgname] [ver] [obfus]",
      description:
        "Build APK with the specified package name, version, and obfuscation options",
    },
    {
      label: "!clean",
      description: "Clean .edit folder and other extracted DroidScript assets",
    },
    {
      label: "!reset [options]",
      description:
        "Reset DroidScript to clean install but keeping all projects.",
      detail:
        "Clears Plugins, Extensions and with 'full' option even saved settings",
    },
    { label: "!exit", description: "Exit DroidScript" },
    {
      label: "$netstat",
      description: "Display network status from android shell",
    },
...
async function addTypes(item) {
  ...
  try {
    let jsconfig = fs.readFileSync(
      homePath(CONSTANTS.DEFINITIONS, "default_jsconfig.json"),
      "utf8"
    );
...
async function autoFormat(item) {
  ...
  try {
    ...
    fs.copyFileSync(
      homePath(CONSTANTS.DEFINITIONS, "settings.json"),
      settingsPath
    );
  } catch (e) {
    catchError(e);
...
async function smartDeclareVars(file) {
  ...
  if (!(uri instanceof vscode.Uri)) {
    const info = await ext.getProjectInfo(
      uri.path || "",
      uri.title,
      async (p) => fs.existsSync(p)
    );
    if (!info)
      return vscode.window.showWarningMessage(
...
async function checkUnsavedChanges() {
  const changes = [
    ...
    ...filesToRename.map(
      (f) =>
        `rename ${getRemotePath(f.oldUri.fsPath)} to ${getRemotePath(
          f.newUri.fsPath
        )}`
    ),
  ];
  ...
  const selection = await vscode.window.showWarningMessage(
    "Apply unsaved changes to remote project?",
    {
      modal: true,
      detail: changes.join("\n"),
    },
...
async function openProjectFolder(proj, sync = true) {
  ...
  if (!exists && sync) {
    ...
    info = await ext.getProjectInfo(proj.path, proj.PROJECT, async (p) =>
      fs.existsSync(p)
    );
  }

  const isOpen = vscode.workspace.workspaceFolders?.find(
    (ws) => ws.uri.fsPath === proj.path
  );
  if (!isOpen) {
    const selection = await vscode.window.showInformationMessage(
      `Open '${proj.PROJECT}'?`,
    ...
    const success = vscode.workspace.updateWorkspaceFolders(n, 0, {
      uri: folderPath,
      name: proj.PROJECT,
    });
    if (!success)
